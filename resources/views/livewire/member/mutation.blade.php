<div class="h-screen bg-gray-50 flex flex-col relative overflow-hidden font-sans">
    ...
    <div class="bg-purple-700 px-5 pt-5 pb-4 shadow-lg z-40 flex-none">
        ...
        <div class="flex p-1 bg-purple-800/40 backdrop-blur-md rounded-xl border border-purple-500/30">
            <button wire:click="$set('filter', 'all')" class="flex-1 py-2 text-xs font-bold rounded-lg transition-all {{ $filter == 'all' ? 'bg-white text-purple-700 shadow-md' : 'text-purple-100' }}">Semua</button>
            <button wire:click="$set('filter', 'deposit')" class="flex-1 py-2 text-xs font-bold rounded-lg transition-all {{ $filter == 'deposit' ? 'bg-white text-purple-700 shadow-md' : 'text-purple-100' }}">Masuk</button>
            <button wire:click="$set('filter', 'withdrawal')" class="flex-1 py-2 text-xs font-bold rounded-lg transition-all {{ $filter == 'withdrawal' ? 'bg-white text-purple-700 shadow-md' : 'text-purple-100' }}">Keluar</button>
        </div>
    </div>

    <div class="flex-1 overflow-y-auto no-scrollbar bg-gradient-to-b from-purple-700 via-gray-50 to-white px-5 pt-4 pb-24">
        
        @forelse($groupedTransactions as $date => $items)
            <div class="mb-6 animate-fade-in">
                <div class="flex items-center gap-2 mb-3">
                    <span class="text-[10px] font-black text-white bg-purple-500/80 px-2 py-0.5 rounded-full uppercase tracking-tighter shadow-sm">
                    ...
                    @foreach($items as $trx)
                        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between transition-all active:scale-95">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center {{ $trx->type == 'deposit' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600' }}">
                                    @if($trx->type == 'deposit')
                            ...
                            <div class="text-right">
                                <p class="text-sm font-black {{ $trx->type == 'deposit' ? 'text-green-600' : 'text-red-600' }}">
