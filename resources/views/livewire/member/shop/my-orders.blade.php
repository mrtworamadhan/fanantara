<div class="h-screen bg-gray-50 flex flex-col relative overflow-hidden font-sans">
    ...
    <div class="bg-purple-700 px-5 pt-5 pb-4 shadow-lg z-40 flex-none">
        <div class="flex items-center gap-3 mb-4">
            ...
            <h1 class="text-xl font-bold text-white tracking-tight">Pesanan Saya</h1>
        </div>

        {{-- Tabs --}}
        <div class="flex gap-1 p-1 bg-purple-800/40 backdrop-blur-md rounded-xl border border-purple-500/30">
            @php
                $tabs = [
                    'all' => 'Semua',
                    'pending' => 'Belum Bayar',
            ...
            @foreach($tabs as $key => $label)
                <button 
                    ...
                    {{ $statusFilter === $key ? 'bg-white text-purple-700 shadow-md' : 'text-purple-100 hover:bg-white/10' }}">
                    {{ $label }}
                </button>
            @endforeach
        </div>
    ...
    <div class="flex-1 overflow-y-auto no-scrollbar bg-gradient-to-b from-purple-700 via-gray-50 to-white px-4 pt-4 pb-24">
            ...
            <div wire:click="showDetail({{ $order->id }})" class="bg-white p-4 rounded-2xl mb-2 shadow-sm border border-gray-100 active:scale-[0.98] transition-transform cursor-pointer">
                ...
                <div class="flex gap-3 items-center border-t border-gray-50 pt-3">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-sm font-black text-purple-600">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
        @empty
            <div class="flex flex-col items-center justify-center py-20 opacity-50 text-center">
                ...
                @else
                    <a href="{{ route('member.marketplace') }}" class="mt-3 text-xs font-bold text-purple-600 hover:underline">Mulai Belanja</a>
                @endif
            </div>
        @endforelse
        ...
        <div class="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
            ...
            <div class="bg-white w-full max-w-lg rounded-t-[2rem] sm:rounded-2xl relative z-10 max-h-[90vh] flex flex-col shadow-2xl animate-slide-up">
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    ...
                    <div class="bg-purple-50 rounded-xl p-4 flex items-center gap-3">
                        <div class="w-10 h-10 bg-purple-100 text-purple-600 rounded-full flex items-center justify-center flex-none">
                ...
                <div class="p-5 border-t border-gray-100 bg-gray-50 rounded-b-3xl">
                    <div class="flex justify-between items-center mb-0">
                        ...
                        <span class="text-lg font-black text-purple-600">Rp {{ number_format($selectedOrder->total_amount, 0, ',', '.') }}</span>
                    </div>
                    ...
                    @if($selectedOrder->status == 'pending')
                        ...
                        <a href="{{ $waUrl }}" 
                           target="_blank" 
                           class="w-full mt-4 py-3 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded-xl shadow-lg shadow-purple-500/20 active:scale-95 transition-all flex items-center justify-center gap-2">
