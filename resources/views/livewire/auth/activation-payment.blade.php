<div class="min-h-screen bg-gray-50 flex flex-col relative pb-10">
    ...
    <div class="h-48 rounded-b-[2.5rem] relative shadow-lg transition-colors duration-500" style="background: {{ $is_rejected ? 'linear-gradient(135deg, #dc2626 0%, #ea580c 100%)' : 'linear-gradient(135deg, #9333ea 0%, #6b21a8 100%)' }};">
        ...
        <div class="bg-white rounded-3xl shadow-xl p-6 mb-6 border border-gray-100">
            ...
            @if(!$is_submitted || $is_rejected)
                <div class="text-center mb-6">
                    ...
                    <div class="flex items-center justify-center gap-1" style="color: #9333ea;">
                ...
                <div class="space-y-4">
                    <div class="bg-gray-50 border border-gray-200 rounded-xl p-4 relative group hover:border-purple-200 transition-all">
                        ...
                        <button onclick="navigator.clipboard.writeText('{{ $bank_account }}'); alert('Nomor rekening disalin!')" class="absolute top-4 right-4 p-2 rounded-lg bg-white shadow-sm border border-gray-100 text-purple-600 hover:bg-purple-50 transition-all">
                ...
                <div class="mt-8">
                    <form wire:submit.prevent="submitPayment" class="space-y-4">
                        <div>
                            ...
                            <label for="proof" class="block w-full cursor-pointer group">
                                <div class="w-full h-40 border-2 border-dashed {{ $is_rejected ? 'border-red-300 bg-red-50' : 'border-gray-300 bg-gray-50' }} rounded-2xl flex flex-col items-center justify-center group-hover:bg-white transition-all relative overflow-hidden">
                                    ...
                                    @else
                                        <div class="w-12 h-12 bg-white rounded-full shadow-sm flex items-center justify-center mb-2 {{ $is_rejected ? 'text-red-500' : 'text-purple-500' }}">
                        ...
                        <button type="submit" wire:loading.attr="disabled" 
                            class="w-full py-4 rounded-xl font-bold text-white shadow-lg transition-all flex justify-center items-center"
                            style="background-color: {{ $is_rejected ? '#dc2626' : '#9333ea' }};"
                            onmouseover="this.style.backgroundColor='{{ $is_rejected ? '#b91c1c' : '#7c3aed' }}'" 
                            onmouseout="this.style.backgroundColor='{{ $is_rejected ? '#dc2626' : '#9333ea' }}'">
            ...
            @else
                <div class="text-center py-8">
                    <div class="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 animate-pulse" style="background-color: #dcfce7;">
                    ...
                    @if(isset(auth()->user()->member->activation_payment_data['proof_path']))
                        <div class="mx-auto w-40 h-40 mt-4 mb-4 border-4 border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                            <img src="{{ asset('storage/' . auth()->user()->member->activation_payment_data['proof_path']) }}" 
            ...
            <div class="mt-8 pt-6 border-t border-dashed border-gray-200 text-center">
                ...
                <a href="https://wa.me/6285158611302?text=Halo%20CS%20Fanantara,%20saya%20{{ urlencode(auth()->user()->name) }}%20(ID:%20{{ auth()->id() }}).%20{{ $is_rejected ? 'Saya%20ingin%20tanya%20soal%20penolakan%20pendaftaran.' : 'Saya%20ingin%20konfirmasi%20pembayaran.' }}" 
                   target="_blank"
                   class="inline-flex items-center justify-center gap-2 w-full py-3 rounded-xl border border-purple-200 bg-purple-50 text-purple-700 font-semibold text-sm hover:bg-purple-100 transition-colors">
